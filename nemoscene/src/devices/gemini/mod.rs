...
impl Gemini {
    fn audio_to_text(audio_data: &[i16]) -> anyhow::Result<String> {
        let filename = format!("/tmp/voice{}.wav", SystemTime::now().duration_since(UNIX_EPOCH).expect("Time went backwards").as_secs());
        let head = wav_io::new_header(16000, 16, false, true);
        let mut file_out = File::create(&filename)?;
        wav_io::write_to_file(&mut file_out, &head, &wav_io::convert_samples_i16_to_f32(&audio_data.to_vec()))?;

        let client = Client::new();
        let url = format!("https://speech.googleapis.com/v1/speech:recognize?key={}", "awesome-key");
